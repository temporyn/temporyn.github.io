...
워크로드 controller는 여러 종류가 있는데, 그중 가장 기본인 **ReplicaSet**과 그 위 계층 **Deployment**를 다룬다.
...
ownerReferences:
  - apiVersion: apps/v1
    ...
    blockOwnerDeletion: true     # Heading.6의 foreground 삭제와 연동
```
...
소유 관계는 `ownerReferences`로 연결된다. Deployment를 지우면 딸린 ReplicaSet과 Pod가 함께 정리된다(cascading delete) — 그 정리 방식은 Heading.6에서 다룬다.
...
| Orphan | 자식은 그대로 살려두고 소유자만 지움. garbage collector가 각 자식의 `ownerReferences`에서 지워진 소유자 항목을 제거해, 자식은 소유자 참조가 아예 없는 고아 상태가 됨(참조가 대상 없이 남는 게 아니다) |
...
비율을 정수로 환산할 때 생기는 나머지(leftover)는 **replicas가 가장 많은 ReplicaSet 하나에 몰아준다** — 여러 RS에 잘게 쪼개 흩뿌리지 않는다. 그리고 이미 `replicas: 0`으로 내려간 ReplicaSet(오래전에 밀려난 옛 리비전 등)은 이 비례 배분 대상에서 **제외되어 다시 스케일 업되지 않는다**. 비율 계산만 따르면 죽은 리비전이 되살아날 수 있는데, 이를 막는 안전장치다.
...
`progressDeadlineSeconds`(기본 **600**) — 이 시간 동안 진전이 없으면 롤아웃을 실패로 표시한다. `kubectl describe deployment`의 Conditions에 `Progressing=False`, reason `ProgressDeadlineExceeded`로 나타난다. Pod가 `ImagePullBackOff`·`CrashLoopBackOff` 등으로 Ready가 되지 못해 롤아웃이 막히는 경우가 대표적인 트리거다.
...
**실패하는 롤아웃과 복구.** 존재하지 않는 이미지 태그로 교체하면 새 Pod가 `ImagePullBackOff`에 걸려 롤아웃이 멈춘다.

```bash
kubectl set image deployment/nginx nginx=nginx:does-not-exist
kubectl rollout status deployment/nginx --timeout=30s   # client가 30초만 대기하고 포기(exit 1)
kubectl get pods -l app=web                              # 새 Pod가 ImagePullBackOff로 멈춰 있음
kubectl describe deployment nginx                         # Conditions: Progressing은 아직 True/ReplicaSetUpdated
kubectl rollout undo deployment/nginx                     # 직전 정상 리비전으로 즉시 복구
```

`--timeout`은 **client 쪽 대기시간**일 뿐, Deployment의 `Progressing` condition에는 영향을 주지 않는다. `Progressing=False`(`ProgressDeadlineExceeded`)는 서버 쪽 `progressDeadlineSeconds`(기본 600초)가 실제로 다 지나야 나타나므로, 30초 뒤에는 아직 `Progressing: True, reason: ReplicaSetUpdated`로 남아 있다. 실습에서 즉시 확인 가능한 실패 신호는 `kubectl get pods`의 `ImagePullBackOff`이고, `Progressing=False` 자체를 재현하려면 `kubectl patch deployment/nginx -p '{"spec":{"progressDeadlineSeconds":30}}'`처럼 데드라인을 짧게 줄인 뒤 기다려야 한다.
...
# 롤링 교체 과정 실시간 관찰(Heading.12 끝 시점 이미지는 nginx:1.27이므로 다른 태그로 바꿔야 실제로 교체가 일어난다)
