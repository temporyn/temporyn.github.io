...
[4. Deployment ↔ ReplicaSet ↔ Pod](#4-deployment--replicaset--pod)  
[5. 업데이트 전략](#5-업데이트-전략)  
[6. 롤아웃과 롤백](#6-롤아웃과-롤백)  
[7. 스케일링](#7-스케일링)  
[8. 주요 필드 정리](#8-주요-필드-정리)  
[9. 실습: 생성·업데이트·롤백](#9-실습-생성업데이트롤백)  
[10. 실습: 스케일과 롤링 관찰](#10-실습-스케일과-롤링-관찰)  
[11. 참고 링크](#11-참고-링크)
...
소유 관계는 `ownerReferences`로 연결된다. Deployment를 지우면 딸린 ReplicaSet과 Pod가 함께 정리된다(cascading delete).
...
| `strategy.rollingUpdate.maxUnavailable` | 25% | 동시 불가용 허용치 |
| `strategy.rollingUpdate.maxSurge` | 25% | 초과 생성 허용치 |
...
kubectl delete -f deploy.yaml       # Deployment 삭제 → RS·Pod 함께 정리
```
