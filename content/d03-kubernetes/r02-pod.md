...
Pod의 네트워크·IPC Namespace를 실제로 붙드는 건 **pause 컨테이너**다. 컨테이너 런타임이 Pod를 만들 때 가장 먼저 이 pause 컨테이너를 띄워 새 Network·IPC Namespace를 생성하고, 이후 만드는 앱 컨테이너들은 자기 Namespace를 새로 만드는 대신 이 pause 컨테이너의 Namespace에 join한다.

pause 프로세스는 아무 일도 하지 않고 그냥 대기(sleep)만 하며, 이미지 크기는 1MB 미만이다. 앱 컨테이너가 크래시해 kubelet이 재생성해도 pause 컨테이너는 그대로 남아 있어 Pod IP가 바뀌지 않는다 — pause 컨테이너 자체가 죽는 드문 경우에만 Pod 전체가 재생성된다.
...
Guaranteed Pod는 `kubepods-besteffort.slice`·`kubepods-burstable.slice` 같은 중간 slice 없이 `kubepods.slice` 바로 아래에 놓인다.

컨테이너 하나하나의 requests·limits뿐 아니라 이 Pod 단위 cgroup 자체에도 집계된 상한이 걸린다. cgroup을 실제로 만드는 방식(cgroupfs 직접 조작 vs systemd 위임)은 kubelet의 `cgroupDriver` 설정으로 정해지고, 노드의 컨테이너 런타임과 이 값이 어긋나면 같은 트리를 두 관리자가 따로 인식해 자원 압박 상황에서 오작동할 수 있다.
...
| 종류 | 동작 |
|:-----|:-----|
| hard | 즉시 축출. `terminationGracePeriodSeconds`·PodDisruptionBudget 모두 무시 |
| soft | 유예 시간(`eviction-max-pod-grace-period`) 뒤 축출. graceful 종료 절차를 존중 |

이 축출도 BestEffort → Burstable → Guaranteed 순으로 우선 진행된다.
...
API 서버를 거치지 않고 **노드의 kubelet이 직접 관리**하는 Pod가 스태틱 Pod다. 노드의 지정 디렉터리(기본 `/etc/kubernetes/manifests`, kubelet의 `--pod-manifest-path`로 변경 가능)에 YAML을 두면 kubelet이 그 파일을 감시해 직접 컨테이너를 띄운다.
